...
$(IWL_ANDROID_SRC_BASE)/$(iwlwifi_module_file):$(mod_cleanup) $(TARGET_PREBUILT_KERNEL)
	$(MAKE) -C $(IWL_ANDROID_SRC_BASE) O=$(IWL_LINUXPATH) ARCH=arm CROSS_COMPILE=$(IWL_CROSS_COMPILE) KLIB=$(IWL_LINUXPATH) KLIB_BUILD=$(IWL_LINUXPATH) COMPAT_CURDIR=$(IWL_ANDROID_ROOT)/$(IWL_ANDROID_SRC_BASE) defconfig-ar6mx
	$(MAKE) -C $(IWL_ANDROID_SRC_BASE) O=$(IWL_LINUXPATH) ARCH=arm CROSS_COMPILE=$(IWL_CROSS_COMPILE) KLIB=$(IWL_LINUXPATH) KLIB_BUILD=$(IWL_LINUXPATH) COMPAT_CURDIR=$(IWL_ANDROID_ROOT)/$(IWL_ANDROID_SRC_BASE) -j8 modules
